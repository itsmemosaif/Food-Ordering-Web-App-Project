...
//need to specify port to run our server and application from
app.listen(8000);
...
function isProdcutInCart(cart,id){
...
app.post('/add_to_cart',function(req,res){
   ...
   //An object named product is created to store products within key:value pair
   var product = {id:id,name:name,price:price,sale_price:sale_price,quantity:quantity,image:image};
    

   //a predefined user session array is used here to store multiple items in cart
   if(req.session.cart){
        var cart = req.session.cart;

        if(!isProdcutInCart(cart,id)){
            cart.push(product);
        }
        ...
        if(err){
            ...
            con.query(query,[values],(err,result)=>{
                ...
                for(let i=0; i<cart.length; i++){
                    ...
                    var values = [
                        [cart[i].order_id,cart[i].product_id,cart[i].product_name,cart[i].product_price,cart[i].product_image,cart[i].product_quantity,cart[i].order_date]
                    ];
                    con.query(query,[values],(err,result)=>{})
                }
